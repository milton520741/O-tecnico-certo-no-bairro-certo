...
-- Cria estrutura completa para painel administrativo
-- ============================================================

-- ============================================================
-- SUPER ADMIN REGISTRY
-- Identifica quais admins são "super admins" com permissões completas
-- ============================================================
...
CREATE TABLE public.admin_logs (
  ...
  action TEXT NOT NULL, -- 'user_banned', 'subscription_approved', etc.
  entity_type TEXT NOT NULL, -- 'user', 'subscription', 'service', etc.
  entity_id UUID,
  entity_data JSONB, -- dados antes da ação
...
-- Service Category Junction (link services to categories)
CREATE TABLE public.service_category_junction (
...
CREATE TABLE public.admin_notifications (
  ...
  type TEXT NOT NULL, -- 'pending_subscription', 'user_banned', 'report', etc.
  title TEXT NOT NULL,
  description TEXT,
  related_id UUID, -- user_id, subscription_id, etc.
...
CREATE TABLE public.user_reports (
  ...
  reason TEXT NOT NULL, -- 'fraud', 'harassment', 'inappropriate_content', etc.
...
CREATE TABLE public.admin_dashboard_config (
  id BIGSERIAL PRIMARY KEY,
  admin_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  widget_type TEXT NOT NULL, -- 'stats', 'recent_users', 'pending_subscriptions', etc.
...
-- Adicionar campos de admin a technicians
...
-- Adicionar campos de admin a companies
...
-- Adicionar campos a subscriptions
...
-- Adicionar campos a payment_proofs
...
-- Function: Check if user is admin (regular or super)
...
-- Function: Check if user is moderator (moderator, admin, or super_admin)
...
ALTER TABLE public.service_category_junction ENABLE ROW LEVEL SECURITY;
...
-- super_admins: only super_admin can read
...
-- admin_settings: public read, only super_admin can modify
...
-- service_categories: public read (active only), admin can modify
...
-- service_category_junction: depends on services/categories visibility
CREATE POLICY "service_category_junction read" ON public.service_category_junction FOR SELECT
  USING (true);

CREATE POLICY "service_category_junction admin manage" ON public.service_category_junction FOR ALL
  USING (public.is_admin_or_higher(auth.uid()))
  WITH CHECK (public.is_admin_or_higher(auth.uid()));

-- admin_notifications: only recipient and super_admin can read
...
-- user_reports: admins can read/manage
...
-- admin_dashboard_config: only user can read/modify own config
...
-- Update zones to allow super_admin to manage (already allowed via 'admin')
-- Update services to allow super_admin to manage (already allowed via 'admin')

-- Add policies for payment_proofs to protect them with RLS
...
INSERT INTO public.service_category_junction (service_id, category_id)
SELECT s.id, sc.id FROM public.services s, public.service_categories sc
WHERE (s.slug IN ('eletricista', 'canalizador', 'pedreiro', 'pintor', 'carpinteiro', 'serralheiro', 'vidraceiro', 'soldador') AND sc.slug = 'construcao')
   OR (s.slug = 'limpeza' AND sc.slug = 'limpeza')
   OR (s.slug = 'eletrodomesticos' AND sc.slug = 'reparacao')
   OR (s.slug = 'mecanico-auto' AND sc.slug = 'automovel')
   OR (s.slug = 'tecnico-informatico' AND sc.slug = 'informatica')
